...
export default () => {
  const s = scene();
  s.box('svc', 260, -90, 240, 80, 'Order service', { fill: 'green' });
  ...
  s.arrow('svc', 'orders', { fromSide: 'b', toSide: 't', t0: 0.25, t1: 0.5, label: 'INSERT / UPDATE\n/ DELETE', labelFs: 17 });
  s.arrow('svc', 'outbox', { fromSide: 'b', toSide: 't', t0: 0.75, t1: 0.5, label: 'INSERT message', labelAt: [660, 150], labelFs: 17 });
  s.box('relay', 1010, 305, 220, 100, 'Message relay', { fill: 'purple' });
  s.box('broker', 1480, 305, 220, 100, 'Message broker', { fill: 'yellow' });
  s.arrow('relay', 'outbox', { fromSide: 'l', toSide: 'r', label: '2. reads\nOUTBOX', labelFs: 17 });
